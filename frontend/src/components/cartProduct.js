    ...
    <div className='bg-slate-200 p-2 flex gap-4 rounded border border-slate-300'>
       ...
       <div className='flex flex-col gap-1 w-full'>
         <div className='flex justify-between'>
          ...
          <button onClick={()=>dispatch(increaseQty(id))} className='bg-slate-300 py-1  mt-2 rounded hover:bg-slate-400 p-1'>+</button>
          <p className='font-semibold p-1'>{qty}</p>
          <button onClick={()=>dispatch(decreaseQty(id))} className='bg-slate-300 py-1  mt-2 rounded hover:bg-slate-400 p-1'>-</button>
          </div>
